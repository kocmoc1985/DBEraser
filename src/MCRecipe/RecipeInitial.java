...
//        });
    ...
    public void addToCompare() {
        ...
        if (ADDED_TO_COMPARE_LIST.size() >= 6) {
            HelpA.showNotification(MSG.LANG("Maximum of 6 recipes reached"));
            return;
        }
        //
...
//        System.out.println("ADD TO COMPARE: " + x);
        //
        if (x == false) {
            HelpA.showNotification(MSG.LANG("No records found for recipe: ") + recipeVersion);
            return;
        }
        //
    ...
    public void delete_record_table_1(JTable table) {
        ...
        if (checkIfRecipeDisabled()) {
            HelpA.showNotification(MSG.LANG("Cannot delete Recipe with status S, O, Inactive"));
            return;
        }
        //
        if (HelpA.confirm() == false) {
    ...
    public void allowIngredSearch(JCheckBox box) {
        ...
        if (box.isSelected()) {
            if (condition_2_3 && condition_2_4) {
                HelpA.showNotification(MSG.LANG("RECIPE ORIGIN or RECIPE VERSION must be chosen before using this option!"));

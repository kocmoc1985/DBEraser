...
//        casno.setUneditable();
    ...
    public void deleteTradeNameFromTable3B() {
        ...
        if (currentId.isEmpty()) {
            HelpA.showNotification(MSG.LANG("Nothing chosen, click on a row in the table below"));
            return;
        }
        //
        if (HelpA.confirm(MSG.LANG("Dou you really want to delete tradename: ") + currentTradeName) == false) {
            return;
        }
        //
    ...
    public void deleteFromTable3_2() {
        ...
        if (currentTradeName.isEmpty()) {
            HelpA.showNotification(MSG.LANG("Nothing chosen plese click on any of corresponding rows"));
            return;
        }
        //
        if (HelpA.confirm(MSG.LANG("Delete tradename: ") + currentTradeName + "?") == false) {
            return;
        }
        //
        ...
        try {
            ...
            HelpA.showNotification(MSG.LANG("Operation failed, because of sql error"));
            Logger.getLogger(Vendors.class.getName()).log(Level.SEVERE, null, ex);
        }
        //
...
//        }
        ...
        //
        if (trade_name == null) {
            HelpA.showNotification(MSG.LANG("Error, please try aggain"));
            return;
        }
        //
...
//        }
    ...
    public void deleteFromTable4() {
        //
        if (HelpA.confirm(MSG.LANG("Delete Vendor: ") + currentVendorId + " ?") == false) {
            return;
        }
        //
...
//        clearAllRowsTableInvert(TABLE_INVERT_4_2);
    ...
    public void addToTable4_2() {
        ...
        if (contactName.isEmpty() && contactName.isEmpty() && position.isEmpty() && phone.isEmpty() && email.isEmpty()) {
            HelpA.showNotification(MSG.LANG("Please fill in the fields before adding"));
            return;
        }
        //
    ...
    public void deleteFromTable4_2() {
        ...
        if (currentVendorContactId.isEmpty()) {
            HelpA.showNotification(MSG.LANG("Contact not chosen, click on any field corresponding to the contact"));
            return;
        }
        //
        if (HelpA.confirm(MSG.LANG("Delete contact: ") + currentVendorContactName) == false) {
            return;
        }
        //
...
//        HelpA.ComboBoxObject boxObject = (HelpA.ComboBoxObject) mCRecipe.jComboBoxVenorsTradnames.getSelectedItem();
        ...
        if (ingred_code_id == null) {
            HelpA.showNotification(MSG.LANG("Cannot proceed,Please choose ingredient"));
            return;
        }
        //

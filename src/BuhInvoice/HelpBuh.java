...
//        map.put(DB.BUH_LICENS__USER, emailUserName);
    ...
    private static String buildUrl(String phpScriptName, String phpFunctionName, String json) {
        if (HTTPS) {
            return String.format("https://" + getDomain() + "/php/%s?%s=true&json=%s", phpScriptName + ".php", phpFunctionName, json);
        } else {
            return String.format("http://" + getDomain() + "/php/%s?%s=true&json=%s", phpScriptName + ".php", phpFunctionName, json);
        }
    }
